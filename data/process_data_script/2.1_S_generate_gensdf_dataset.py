...
def convert_mesh(ply_file, clear_temp, wt_method, sdf_method, sample_method=['random', 'random']):
    ...
    temp_dir   = Path(f'../datasets/2_gensdf_summarized_dataset/temp/{stem}')
    result_dir = Path(f'../datasets/2_gensdf_summarized_dataset/')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_sample_point', type=int, default=40000, help='Number of points for each mesh')
    parser.add_argument('--uniform_sample_ratio', type=float, default=0.5, help='Uniform sample ratio')
    parser.add_argument('--n_point_cloud', type=int, default=10000, help='Number of point cloud')
    ...
    shutil.rmtree('../datasets/2_gensdf_summarized_dataset', ignore_errors=True)

    all_ply_files = list(filter(lambda x : x.as_posix()[-3:] == 'ply' and "Storage" in x.as_posix(),
